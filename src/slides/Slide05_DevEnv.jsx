...
export default function Slide05_DevEnv() {
  ...
  const copy = {
    ko: {
      title: '최소주의 개발 환경',
      subtitle: '꼭 필요한 도구만, 최대한 단순하게',
      leftHeading: '🏗️ Core Engine',
      rightHeading: '🤖 Claude Code 생태계',
      coreItems: [
    ...
    en: {
      ...
      subtitle: 'Only the tools you truly need, kept intentionally simple',
      leftHeading: '🏗️ Core engine',
      rightHeading: '🤖 Claude Code stack',
      coreItems: [
        { icon: '🤖', name: 'Claude Code', desc: 'CLI AI coding assistant', tag: 'Core' },
        { icon: '📦', name: 'Node.js / npm', desc: 'Package management and build tooling', tag: 'Runtime' },
        { icon: '🌿', name: 'Git', desc: 'Version control and history', tag: 'VCS' },
        { icon: '🖥️', name: 'VS Code / Cursor', desc: 'The day-to-day editor', tag: 'Editor' },
      ],
      claudeItems: [
        { icon: '📋', name: 'AGENTS.md', desc: 'Project rules and AI instructions', tag: 'Context' },
        { icon: '⚙️', name: 'CLAUDE.md', desc: 'Global Claude behavior settings', tag: 'Global' },
        { icon: '🔌', name: 'MCP servers', desc: 'Tool extensions and external integrations', tag: 'Extension' },
        { icon: '💡', name: 'Custom Skills', desc: 'Reusable workflow units', tag: 'Workflow' },
      ],
    },
  };
  const t = copy[locale];
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {t.coreItems.map((item) => <EnvCard key={item.name} item={item} colorClass="tag-purple" />)}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {t.claudeItems.map((item) => <EnvCard key={item.name} item={item} colorClass="tag-cyan" />)}
            </div>
          </div>
        </div>
      </div>
